...
async function conexao_bd_servicos(){
	const{data, error} = await banco.from("servicos").select("id, nome, servico, data, hora, pagamento, parcelas, valor, status").order("nome", { ascending: true });
...
function validar_agenda(){
	...
	//Exibe o texto de formulario enviado corretamente
...
async function enviar_forms(){
	if(validar_agenda() == true){
		...
		if(!map_nomes().includes(agenda_nome.value)){
